...
class WC_Product_Variable_Data_Store_CPT_Test extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox sync_managed_variation_stock_status backfills unmanaged children that have no _stock_status meta row.
	 */
	public function test_sync_managed_variation_stock_status_backfills_children_with_missing_status(): void {
		...
		foreach ( $child_ids as $child_id ) {
			update_post_meta( $child_id, '_manage_stock', 'no' );
			// Simulate a variation created outside WC's save pipeline: no _stock_status row.
			$wpdb->delete(
				$wpdb->postmeta,
				array(
					'post_id'  => $child_id,
