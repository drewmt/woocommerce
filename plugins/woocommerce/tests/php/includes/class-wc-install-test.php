...
class WC_Install_Test extends \WC_Unit_Test_Case {
	...
	public function test_db_update_callbacks_versions(): void {
		...
		// An unreleased build (-dev/-rc) can't go past the version being developed. A clean version can
		// also allow the next patch, since a release branch collects that patch's callbacks before it
		// ships (a release build check validates the exact patch).
